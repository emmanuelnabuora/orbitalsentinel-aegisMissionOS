...
async def create_mission(body: MissionCreate, session: SessionDep, actor: Operator, ws: WorkspaceDep) -> MissionRead:
    mission = await MissionService(session).create(body, actor_id=actor.id, workspace_id=ws.id if ws else None)
...
async def list_missions(
    ...
    missions, total = await MissionService(session).repo.list(
        ...
        allowed_markings=visible_markings(viewer.clearance), offset=offset, status=mission_status
    )
    return Page(
...
async def get_mission(mission_id: uuid.UUID, session: SessionDep, viewer: CurrentUser) -> MissionRead:
...
async def mission_graph(mission_id: uuid.UUID, session: SessionDep, viewer: CurrentUser) -> MissionGraph:
    mission = await MissionService(session).get(mission_id)
    if not can_read(viewer, mission):
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Mission not found")
