...
from aegis_api.core.abac import dominates, visible_markings
...
async def test_classified_assets_filtered_from_lists(client, admin, make_user):
    ...
    names = [
        a["name"]
        for a in (await client.get("/api/v1/assets", headers=viewer_headers)).json()["items"]
    ]
...
async def test_clearance_grant_reveals(client, admin, make_user):
    ...
    names = [
        a["name"]
        for a in (await client.get("/api/v1/assets", headers=analyst_headers)).json()["items"]
    ]
...
async def test_cui_alert_visibility(client, admin, make_user):
    ...
    titles = [
        a["title"]
        for a in (await client.get("/api/v1/alerts", headers=low_headers)).json()["items"]
    ]
    ...
    assert (
        await client.get(f"/api/v1/alerts/{alert_id}", headers=admin_headers)
    ).status_code == 404
...
async def test_admin_needs_clearance_too(client, admin):
    ...
    await client.put(
        f"/api/v1/users/{me.id}/clearance", json={"clearance": "secret"}, headers=headers
    )
    r = await client.get(f"/api/v1/assets/{secret['id']}", headers=headers)
    assert r.status_code == 200
